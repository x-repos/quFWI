...
def FBPINN_update(optimiser_fn, active_opt_states,
    ...
    # update step (has_aux=True: loss_fn returns (total_loss, components_dict))
    (lossval, loss_components), grads = value_and_grad(FBPINN_loss, argnums=0, has_aux=True)(
    ...
    return lossval, loss_components, active_opt_states, active_params
...
def PINN_update(optimiser_fn, active_opt_states,
    ...
    static_params = combine(static_params_dynamic, static_params_static)
    # update step (has_aux=True: loss_fn returns (total_loss, components_dict))
    (lossval, loss_components), grads = value_and_grad(PINN_loss, argnums=0, has_aux=True)(
...
def _common_train_initialisation(c, key, all_params, problem, domain):
    ...
    # Check if custom optimiser is provided, otherwise use default adam
    if callable(c.optimiser):
        optimiser = c.optimiser(**c.optimiser_kwargs) if c.optimiser_kwargs else c.optimiser()
    else:
...
class FBPINNTrainer(_Trainer):
    ...
    def train(self):
        ...
        for i,active_ in enumerate(scheduler):
            ...
            lossval, self._loss_components, active_opt_states, active_params = update(active_opt_states,
    ...
    def _test(self, x_batch_test, u_exact, u_test_losses, x_batch, test_inputs, i, pstep, fstep, start0, active, all_params, model_fns, problem, decomposition):
        ...
        u_std = u_exact.std().item()
        l1n = l1 / u_std if u_std > 0 else 0.0  # Handle zero exact solution
