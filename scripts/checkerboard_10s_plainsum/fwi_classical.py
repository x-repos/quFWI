"""
Full Waveform Inversion (FWI) — Checkerboard + 10-source plain summation

Uses 10 SPECFEM point sources superposed with equal weights (+1),
following Rasht-Behesht et al. (2022) Case 5. The acoustic wave equation
is linear so the sum of all 10 wavefields is a valid solution with the
SAME velocity. One PINN learns the combined wavefield.

  - Scalar potential formulation: network outputs phi(x',z',t)
  - Coordinate scaling Lx=Lz=3
  - Soft ICs from SPECFEM wavefield snapshots (summed over 10 sources)
  - Free-stress BC: P=0 at top surface z'=az/Lz
  - 10 sources x 20 receivers, plain summation
...
# Box with ~50m margin: x=[0.80, 1.25] km, z=[0.10, 0.45] km in SPECFEM coords
# Shift by absorbing layer offset to get PINN coords
z_st_phys = 0.10 - n_absz * dz_grid   # 0.05 km
z_fi_phys = 0.45 - n_absz * dz_grid   # 0.40 km
x_st_phys = 0.80 - n_absx * dx_grid   # 0.65 km
...
L_F = 50  # subsample every 50 steps from SPECFEM (finer for 60 Hz)
...
# Data directory (10-source data)
DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                        "data", "checkerboard_10s", "specfem")


# ============================================================================
# DATA LOADING (10-source plain summation)
# ============================================================================

def _load_single_event(event_dir, xz, xxzzs):
    """Load IC wavefields and seismograms for one source event."""
    wf_dir = os.path.join(event_dir, "wavefields")
    wf_files = sorted(f for f in os.listdir(wf_dir) if f.startswith("wavefield0"))
    ...
    Sz = np.concatenate([s[:, 1:2] for s in seismo_listz], axis=0) / u_scl
    ...
    Sx = np.concatenate([s[:, 1:2] for s in seismo_listx], axis=0) / u_scl

    return {
        "U_ini1x": U_ini1x, "U_ini1z": U_ini1z,
        "U_ini2x": U_ini2x, "U_ini2z": U_ini2z,
        "U_specx": U_specx, "U_specz": U_specz,
        "Sx": Sx, "Sz": Sz,
        "l_sub": l_sub, "t_spec_sub": t_spec_sub,
    }


def load_specfem_data(data_dir=None):
    """Load SPECFEM data for all 10 source events and sum with equal weights.

    Plain superposition (all +1 weights) following Rasht-Behesht et al. (2022).
    """
    if data_dir is None:
        data_dir = DATA_DIR

    # --- Shared grid setup (from first event) ---
    event1_dir = os.path.join(data_dir, "event1")
    X0 = np.loadtxt(os.path.join(event1_dir, "wavefields", "wavefield_grid_for_dumps_000.txt"))
    X0 = X0 / 1000  # m -> km
    X0[:, 0:1] = X0[:, 0:1] / Lx
    X0[:, 1:2] = X0[:, 1:2] / Lz
    ...
    # --- Load and sum all events (plain +1 superposition) ---
    sum_data = None
    for ev in range(1, n_event + 1):
        event_dir = os.path.join(data_dir, f"event{ev}")
        print(f"  Loading event {ev}/{n_event} from {event_dir}")
        ev_data = _load_single_event(event_dir, xz, xxzzs)
        if sum_data is None:
            sum_data = ev_data
        else:
            for key in ["U_ini1x", "U_ini1z", "U_ini2x", "U_ini2z",
                         "U_specx", "U_specz", "Sx", "Sz"]:
                sum_data[key] = sum_data[key] + ev_data[key]

    l_sub = sum_data["l_sub"]
    t_spec_sub = sum_data["t_spec_sub"]

    # --- Seismometer coordinates (shared across sources) ---
    d_s = np.abs(zl_s - z0_s) / (n_seis - 1)
    n_seis_pts = n_seis * l_sub
    X_S = np.empty([n_seis_pts, 3])
    ...
    return {
        "X_init1": X_init1, "U_ini1x": sum_data["U_ini1x"],
        "U_ini1z": sum_data["U_ini1z"],
        "X_init2": X_init2, "U_ini2x": sum_data["U_ini2x"],
        "U_ini2z": sum_data["U_ini2z"],
        "X_S": X_S, "Sx": sum_data["Sx"], "Sz": sum_data["Sz"],
        "X_BC_t": X_BC_t,
        "U_specx": sum_data["U_specx"], "U_specz": sum_data["U_specz"],
        "xx": xx, "zz": zz,
        "l_sub": l_sub,
    }
...
class AcousticFWIScalarPotential(Problem):
    """Acoustic FWI using scalar potential formulation (checkerboard model).
...
def alpha_true_fn(x_phys, z_phys):
    """Compute the true acoustic wavespeed (3x3 checkerboard anomaly).

    Args:
        x_phys, z_phys: physical coordinates in km (NOT scaled).
                         These are PINN-domain coords (absorbing layers removed).
    """
    # Checkerboard in SPECFEM coords: x=[0.85, 1.20] km, z=[0.15, 0.40] km
    # Convert to PINN coords by subtracting absorbing offsets
    cb_x0 = 0.85 - n_absx * dx_grid
    cb_x1 = 1.20 - n_absx * dx_grid
    cb_z0 = 0.15 - n_absz * dz_grid
    cb_z1 = 0.40 - n_absz * dz_grid
    n_cells_x, n_cells_z = 3, 3

    checker = np.sin(n_cells_x * np.pi * (x_phys - cb_x0) / (cb_x1 - cb_x0)) * \
              np.sin(n_cells_z * np.pi * (z_phys - cb_z0) / (cb_z1 - cb_z0))

    # Smooth mask
    mask_x = 0.5 * (1 + np.tanh(200 * (x_phys - cb_x0))) * \
             0.5 * (1 + np.tanh(200 * (cb_x1 - x_phys)))
    mask_z = 0.5 * (1 + np.tanh(200 * (z_phys - cb_z0))) * \
             0.5 * (1 + np.tanh(200 * (cb_z1 - z_phys)))
    mask = mask_x * mask_z

    return 3.0 + 0.3 * checker * mask
...
class FBPINNTrainerFWI16(FBPINNTrainer):
    """FBPINNTrainer subclass for checkerboard FWI with custom test plots."""
...
def main(resume_total_steps=0):
    ...
    cache_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                             'results', 'checkerboard_10s_plainsum', 'cache')
    ...
    if resume_total_steps > 0:
        ...
        # Load SPECFEM data
        print("=" * 60)
        print(f"Step 1: Loading SPECFEM data from {n_event} sources (plain summation)")
        print("=" * 60)
    ...
    # Override results path to results/checkerboard_10s_plainsum/
    c.results_base = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                                   'results', 'checkerboard_10s_plainsum')
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Checkerboard 10-source plain summation FWI on FBPINNs")
